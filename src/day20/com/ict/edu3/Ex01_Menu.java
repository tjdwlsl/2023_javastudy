...
//		Dimesion ds = Toolkit.getDefaultToolkit().getScreenSize();
//		setBounds(ds.width/2-300, ds.height/2-150, 500, 600);
	...
	public void itemStateChanged(ItemEvent e) {
		...
		Enumeration<AbstractButton> bg1_list1 = bg1.getElements();
		while (bg1_list1.hasMoreElements()) {
			// bg1의 요소를 꺼내서 JRadioButton으로 형변환
			AbstractButton abstractButton = (AbstractButton) bg1_list1.nextElement();
		...
		// bg1에 존재하는 요소들을 배열(?)로 만든다.
		Enumeration<AbstractButton> bg2_list2 = bg2.getElements();
		while (bg2_list2.hasMoreElements()) {
			// bg1의 요소를 꺼내서 JRadioButton으로 형변환
			AbstractButton abstractButton = (AbstractButton) bg2_list2.nextElement();
			JRadioButton jb = (JRadioButton) abstractButton;
			if (jb.isSelected()) {
				if (jb == jrb4) {
					price = price + 500;
	...
	// bg1에 존재하는 요소들을 배열(?)로 만든다.
	Enumeration<AbstractButton> bg3_list3 = bg3.getElements();
	while(bg3_list3.hasMoreElements())
	{
		// bg1의 요소를 꺼내서 JRadioButton으로 형변환
		AbstractButton abstractButton = (AbstractButton) bg3_list3.nextElement();
		JRadioButton jb = (JRadioButton) abstractButton;
		if (jb.isSelected()) {
			if (jb == jrb8) {
				price = price * 0.8;
