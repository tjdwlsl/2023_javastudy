...
public class Ex06_List {
	...
	// add(index, E) => 삽입
	// pop : 맨 위에 존재하는 객체를 반환하게 삭제 한다.
	// peek : 맨 위에 존재하는 객체를 반환. 삭제 한함
	// search : 검색(오른쪽, 1부터) => 스택 관점
	// **(추천)indexOf : 검색(왼쪽, 1부터) => 배열 관점
	// elementAt(index) : 요소 반환
	// **get(index) : 요소 반환
	// firstElement() : 맨 처음요소 반환
	// lastElement() : 맨 마지막요소 반환
	...
	public static void main(String[] args) {
		...
		// 둘리를 가지고 있나
		if (stack.contains("둘리")) { // 둘리가 있으면 true (그럼실행함)
			// 둘리 위치 찾기
