...
//이벤트 리스너 : 이벤트 감지, 이벤트가 발생되면 실제적인 이벤트를 처리할 수 있도록
//			   만든 인터페이스
//이벤트 핸들러 : 리스너에서 전달된 이벤트를 실제로 처리할 수 있도록 이벤트 리스너에
//			   포함되어 있는 메소드
//			   즉 발생된 이벤트 객체를 받아서 실제적으로 처리해주는 기능을 하는 메서드

//ActionEvent : 컴포넌트를 눌렀거나, 선택되었을때, Enter킬를 눌렀을 때 발생하는 메서드
//ActionListener : actionPerformed() 추상메서드를 가짐
//addActionListener : 컴포넌트에 actionListener를 추가해서 이벤트를 감지할 수 있
//         			  도록 하는 메서드
...
//        setBounds(ds.width/2-150, ds.height/2-50, 300, 100);
    ...
    // ActionListener 일때 이벤트 처리를 받아서 실제 처리해주는 메서드
    @Override
    public void actionPerformed(ActionEvent e) {
        // System.out.println("버튼을 눌렀네요");
        JButton obj = (JButton) e.getSource();
        if(obj == jb1) {
            System.out.println("입력1이 눌렸음");
        }else if(obj == jb2) {
            System.out.println("입력2이 눌렸음");
        }else if(obj == jb3) {
            System.exit(0);
        }
    }
