...
//스레드(일꾼) : 프로세스(시행하고 있는 프로그램) 내에서 실행되는 세부  작업 단위
//싱글 스레드 : 하나의 프로세스에 하나의 스레드가 일처리 하는것 (day01~day18)
//멀티 스레드 : 하나의 프로세스에서 여러개의 스레드가 병행적으로 처리되는 것
//자바에서 스레드 생성 법
//자바에서는 start()를 호출하면 run()이 실행 되면 스레드 처리 됐다함
//1.Thread 클래스 상속 받기 : start(), run() 메서드가 모두 있다
//2.Runnable 인터페이스 구현 : run만 존재함
...
public class Ex01_Thread {
	...
	public static void main(String[] args) {		
		//지금은 싱글스레드 (즉, 하나의 스레드가 모든일을 다 처리함)
