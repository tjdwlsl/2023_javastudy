...
public class ChatClient extends JFrame implements Runnable{
	...
	public ChatClient() {
		...
		JButton login_bt = new JButton("입장하기");
		jp1_2.add(login_bt);
		...
		user_list = new JList();
		JScrollPane jsp = new JScrollPane(); //리스트가 많아질경우 스크롤 사용
		...
		room_list = new JList();
		room_list.setFont(new Font("궁서체", Font.BOLD, 20));
		...
		login_bt.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(name.length()>0) {
					...
					JOptionPane.showMessageDialog(getParent(), "닉네임 입력하셈요");				}
					nickname_tf.setText("");
	...
	private boolean connected() {
		boolean value = true;
		try {
			s = new Socket("192.168.0.56", 7779);
			...
			try {
				...
				if(obj != null) {
					...
					case 4: //방 참여
						...
						break;
					case 6: //쪽지보내기
						JOptionPane.showInternalMessageDialog(getParent(), p.getMsg());
						break;		
					}
				}
			} catch (Exception e) {
