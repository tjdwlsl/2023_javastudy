...
//파싱 : 구분분석, 문장을 이루고 구성 성분을 분해하고 분해된 성분의 관계를 분석하여 구조를 결정
//		즉 데이터를 분해 분석하여 원하는 형태로  조립하고, 다시 빼는 과정을 말함
//HTML : Hyper Text MarkUp Language => 하이퍼 텍스트 마크업 언어
//XML : extensible MarkUp Language => 하이퍼 텍스트보다 확장된 마크업 언어
//		사용자가 태그를 만들어서 사용하는 언어
//Tag : <시작태그 속성="값" 속성="값"> 텍스트 </끝 태그>

//XML 파싱 : xml로 구성된 데이터를 원하는 데이터만 추출, 가공하는 것
//DOM(Document Object Modeling) 방식 : 웹 브라우저가 태그를 해석학 표현하는 방식
...
public class Ex04 {
	public static void main(String[] args) {
		...
		//파일쓰기
		String pathname = "D:"+File.separator+"ksj"+File.separator+"오늘날씨.txt";
		...
		try {
			...
			for (int i = 0; i < locals.getLength(); i++) {
				...
				sb2.append(txt+"\t"); //for문읽을때 한줄로 나타내야해..?
				sb2.append(attr1+"\t");
				sb2.append(attr2+"\n");
