...
//카드 레이아웃 : 카드가 뒤집혀 전 상태에서 이벤트가 발생하면 하나씩 보여주는 형태의 레이아웃
//  	       (여기서 카드란 JPanel()이라고 생각하면 됨)
public class Ex01_CardLayout extends JFrame {
	...
	public Ex01_CardLayout() {
		...
		// pg1.add("호출이름", 해당 커넨이너 (카드JPanel));
		// 해당 카드를 호출 할 때 =>> show(cardLayout, "호출이름");
		pg1.add("1", card1);
		pg1.add("2", card2);
		pg1.add("3", card3);
		pg1.add("4", card4);
